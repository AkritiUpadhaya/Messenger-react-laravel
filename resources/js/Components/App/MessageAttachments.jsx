        ...
        {attachments.map((attachment, ind)=>(
            ...
            className={`group flex flex-col items-center justify-center text-gray-500 relative cursor-pointer`+
                ...
                {!isAudio(attachment)&&(
                    ...
                    className='z-20 opacity-100 group-hover:opacity-100 transition-all w-8 h-8 flex items-center justify-center text-gray-100 bg-gray-700 rounded absoulte right-0 top-0 cursor-pointer hover:bg-gray-800'>
                        <ArrowDownTrayIcon className='w-4 h-4 '/>
                    </a>
                )}
                {isImage(attachment)&&(
                    <img src={attachment.url} className='object-contain aspect-square' alt="" />
                )}
                {isVideo(attachment) &&(
                ...
                {!isPreviewable(attachment)&&(
                    <a href={attachment.url} onClick={(ev)=> ev.stopPropagation()}
                    download className='flex flex-col justify-center items-center'>
